...
def fetch_cifar100(root, train=False, transform=None, target_transform=None):
    transform = transform if transform is not None else torchvision.transforms.ToTensor()
    dataset = torchvision.datasets.CIFAR100(root, train=train, transform=transform, target_transform=target_transform,
                                           download=True)
    return dataset


def load_cifar100(args, root, transform=None, target_transform=None, **kwargs):
    train_set = fetch_cifar100(root, train=True, transform=transform, target_transform=target_transform)
    test_set = fetch_cifar100(root, train=False, transform=transform, target_transform=target_transform)
    val_loader = None
...
def load_cifar100_data(args):
    train_loader, test_loader, val_loader = load_cifar100(args=args, root=args.datasets_dir)
...
def get_dataloader(args):
    ...
    data_loader = load_cifar100_data(args)
    return data_loader
